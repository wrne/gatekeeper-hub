...
router.get('/alias', async (req, res) => {
	...
	try {
		// const products = req.query.products.split(',').map((product) => product.trim())
		const availabilityList = await getAvailabilityController({produtos: req.query.products, tipo: 'alias'})
		logMessage(`availability list retrieved successful`)
 ...
 * @param {String} produtos - Lista de produtos separados por ponto e vírgula
 */
router.get('/alias/branch', async (req, res) => {
	
	try {
		// const products = req.query.products.split(',').map((product) => product.trim())
		const availabilityList = await getAvailabilityController({produtos: req.query.products, tipo: 'alias_filial'})
		logMessage(`availability list retrieved successful`)
